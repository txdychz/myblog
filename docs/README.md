...
事件委托元素的子元素如果含有多个子元素，且要求点击任意一个子元素都要获取到绑定的元素的数据，获取event.target的父元素。



### 5.uniapp转微信小程序图片异常放大

场景：uniApp项目编译为微信小程序项目，hbuilder和微信开发者工具正常显示，真机运行小程序的时候，第一次进入图片异常放大，刷新后正常显示。
