<template><div><h2 id="get-和-post-区别" tabindex="-1"><a class="header-anchor" href="#get-和-post-区别" aria-hidden="true">#</a> get 和 post 区别</h2>
...
<p>get 和 post 在实现上没有什么区别，都是基于 tcp 协议，但是在 htpt 上 get 可以做的事情，post 也可以做</p>
...
<p>如果第一次握手丢失了，客户端没有接受到服务器的 ack，会触发客户端的超时重传，重新发送 syn，重传的 syn 是一样的
如果第二、第三次握手丢失了，服务器没有接受到客户端的 ack，会触发服务器的超时重传，重新发送 ack</p>
...
<p>反射型需要服务器有对应接口配合</p>
<p>在目标网站的表单中输入恶意代码，一般就是 script 标签，src 属性是黑客网站上的脚本，脚本内容一般就是动态创建一个 img 标
签，标签 src 就是黑客网站上的一个接口，将网站的 cookies 作为参数去请求黑客服务器</p>
...
<ol>
<li>
<p>对 cookies 设置 httponly，使其不能通过 document.cookie 进行获取</p>
</li>
<li>
<p>对用户的输入进行转义，利用第三方库对用户的输入内容进行编码和解码</p>
</li>
<li>
<p>对用户输入的 dom 属性和事件进行过滤，如 script 标签和 onerror 事件</p>
</li>
<li>
<p>后端可以设置 xss 过滤</p>
</li>
...
<li>
<p>富文本编辑器中利用第三方库（安全的标签不进行转义，不安全的标签需要进行转义）</p>
</li>
</ol>
...
a 网站某个接口的 url</p>
<h4 id="防御手段" tabindex="-1"><a class="header-anchor" href="#防御手段" aria-hidden="true">#</a> 防御手段</h4>
<ol>
<li>判断 referer</li>
<li>加验证码</li>
</ol>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="如何取消请求" tabindex="-1"><a class="header-anchor" href="#如何取消请求" aria-hidden="true">#</a> 如何取消请求</h2>
<p>通过创建 AbortController 实例，调用实例的 abort 方法，该接口对 XHR 和 fetch 都有效</p>
